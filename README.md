...
图生图时，在同一条消息中附带一张或多张图片，或者引用一条包含图片的消息，直接用自然语言提出修改要求就可以了。聊天模型会调用 `edit_image` Tool。

`edit_image` Tool 可以选择标准尺寸 `1024x1024`、`1024x1536`、`1536x1024`，也可以传入 `original` 使用第一张参考图的真实尺寸。插件会在每次请求中读取所有参考图片的宽高，并把这些尺寸加入一个尺寸集合供llm选择使用。
